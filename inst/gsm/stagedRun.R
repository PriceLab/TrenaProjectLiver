...
{
   printf("--- runStagedSGM.footprints")
   print(short.spec)
   ...
   stopifnot(targetGene %in% (tbl.geneInfo$geneSymbol))

   #tbl.geneLoc <- tbl.geneInfo[targetGene,]
   tbl.geneLoc <- subset(tbl.geneInfo, geneSymbol==short.spec$geneSymbol)[1,]
   ...
   browser()
   xyz <- "finishing up runStageSGM.footprints"
